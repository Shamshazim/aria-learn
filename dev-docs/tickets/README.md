...
P2H-07 on `feat/P2H-07-sentence-streaming` stacked on it, and P2H-08 on
`feat/P2H-08-voice-identity-and-prosody` stacked on that (no PRs yet). Three tickets are
additionally blocked on human judgement rather than on code: P2H-03 on the `aria.md` persona
review and the >= 80% rubric round in `../golden/tutoring/scores.md`, and P2H-08 on the
listening review in `../voice-review.md` and the still-open provider decision (P2-01).
P2H-09 … P2H-15 are not started. Each ticket's "Status" section says exactly what is left.
